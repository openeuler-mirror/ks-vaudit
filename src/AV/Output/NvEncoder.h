...
public:
	...
	*/
	void EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket, NV_ENC_PIC_PARAMS *pPicParams = NULL);
...
private:
	uint32_t m_nWidth;
	uint32_t m_nHeight;
	uint32_t m_nFrameRateNum;
	uint32_t m_naverageBitRate;
	...
	uint32_t m_nMaxEncodeWidth;
	uint32_t m_nMaxEncodeHeight;
	void* m_hModule;
	int32_t m_iToSend;
	int32_t m_iGot;
	int32_t m_nEncoderBuffer;
	int32_t m_nOutputDelay;
};
