...
NvEncoder::NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void *pDevice, uint32_t nWidth, uint32_t nHeight, NV_ENC_BUFFER_FORMAT eBufferFormat,
							uint32_t nExtraOutputDelay, bool bMotionEstimationOnly, uint32_t nFrameRateNum, uint32_t naverageBitRate) :
	m_pDevice(pDevice), 
	m_eDeviceType(eDeviceType),
	m_nWidth(nWidth),
	m_nHeight(nHeight),
	m_nFrameRateNum(nFrameRateNum),
	m_naverageBitRate(naverageBitRate),
	m_nMaxEncodeWidth(nWidth),
	m_nMaxEncodeHeight(nHeight),
	m_eBufferFormat(eBufferFormat), 
	m_bMotionEstimationOnly(bMotionEstimationOnly), 
...
void NvEncoder::LoadNvEncApi()
{
	...
	// printf("get nvenc api version: %d %d.%d\n", version, version>>4, version|0x0000000F);
...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid)
{
	...
	if (pIntializeParams == NULL || pIntializeParams->encodeConfig == NULL)
	{
	...
	pIntializeParams->frameRateNum = m_nFrameRateNum;
		...
		&& pIntializeParams->presetGUID != NV_ENC_PRESET_LOSSLESS_HP_GUID)
	{
		...
		pIntializeParams->encodeConfig->rcParams.maxQP = {30, 30, 30};
		pIntializeParams->encodeConfig->rcParams.rateControlMode  = NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ;//NV_ENC_PARAMS_RC_VBR;//NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ;//NV_ENC_PARAMS_RC_CBR_HQ;
		pIntializeParams->encodeConfig->rcParams.averageBitRate = m_naverageBitRate;
		pIntializeParams->encodeConfig->rcParams.maxBitRate = 7 * (m_naverageBitRate / 5); // 1.4 times
		pIntializeParams->encodeConfig->rcParams.vbvBufferSize = m_naverageBitRate / (pIntializeParams->frameRateNum / pIntializeParams->frameRateDen);
#endif
	}
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncoderParams)
{
	...
	m_vMappedInputBuffers.resize(m_nEncoderBuffer, NULL);

	m_vpCompletionEvent.resize(m_nEncoderBuffer, NULL);
#if defined(_WIN32)
	...
	{
		m_vMappedRefBuffers.resize(m_nEncoderBuffer, NULL);
		InitializeMVOutputBuffer();
	}
	else
	{
		m_vBitstreamOutputBuffer.resize(m_nEncoderBuffer, NULL);
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, bool bOutputDelay)
{
	...
	for (; m_iGot < iEnd; m_iGot++)
	{
		...
		if (m_vMappedInputBuffers[m_iGot % m_nEncoderBuffer])
		{
			...
			m_vMappedInputBuffers[m_iGot % m_nEncoderBuffer] = NULL;
		}
		...
		if (m_bMotionEstimationOnly && m_vMappedRefBuffers[m_iGot % m_nEncoderBuffer])
		{
			...
			m_vMappedRefBuffers[m_iGot % m_nEncoderBuffer] = NULL;
		}
	}
}
